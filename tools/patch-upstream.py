...
# Production WebAssembly: remove debug-only filesystem/source-map overhead and
# enable SIMD for particle/air loops on modern Yandex-supported browsers.
for debug_arg in (
    "\t\t'-s', 'FS_DEBUG',\n",
    "\t\t'--source-map-base=./',\n",
    "\t\t'-gsource-map',\n",
...
# Use the real Meson release profile (-O3) for the browser build. Disable LTO
# for Emscripten here: this port benefits more from predictable compile/runtime
# behaviour than from the extra link-time pass.
...
# Partial browser updates are one logical simulation frame only after AfterSim.
...
model_perf_text = model_perf_text.replace(
    queued_frame_anchor, queued_frame_patch, 1
)
...
#endif
"""
if "#include <emscripten.h>" not in controller_text:
    ...
    controller_text = controller_text.replace(
        controller_include_anchor, controller_include_patch, 1
    )

# Keep the browser responsive by budgeting actual wall-clock simulation work.
# A few reactive/dense objects can be more expensive than tens of thousands of
# cheap particles, so particle count alone is not a safe scheduling heuristic.
...
if update_anchor not in controller_text:
    raise SystemExit("GameController::Update browser time-budget anchor missing")
...
\t\t# Override any cached Meson value from earlier LTO experiments.
\t\tmeson_configure+=
# Yandex Web/no-HTTP: strip upstream server constants from the generated
# Config.h/WASM entirely. NOHTTP already disables the transport, but leaving
# the default powdertoy.co.uk strings embedded in the binary violates the
